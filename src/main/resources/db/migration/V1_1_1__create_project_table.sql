...
# 1. 공지사항 테이블
create table if not exists happy_home_repair.tb_notice (
...
# 2. 자주하는 질문 테이블
create table if not exists happy_home_repair.tb_faq (
...
# 3. 블로그 게시물 테이블
create table if not exists happy_home_repair.tb_blog_posting_info(
...
create table if not exists happy_home_repair.tb_from_post_type(
...
# 3-1. FK 설정
alter table happy_home_repair.tb_blog_posting_info add foreign key(f_t_no)
    references happy_home_repair.tb_from_post_type(f_t_no) on delete cascade;

###########################################################################################################

# 4. 견적문의 시작
create table if not exists happy_home_repair.tb_repair_apply(
...
#
create table if not exists happy_home_repair.tb_repair_type(
...
#
create table if not exists happy_home_repair.tb_repair_location(
...
#
create table if not exists happy_home_repair.tb_repair_state(
...
# 4-1. FK 설정
alter table happy_home_repair.tb_repair_apply add foreign key(r_type_no)
    references happy_home_repair.tb_repair_type(r_type_no) on delete cascade;

alter table happy_home_repair.tb_repair_apply add foreign key(r_location_no)
    references happy_home_repair.tb_repair_location(r_location_no) on delete cascade;

alter table happy_home_repair.tb_repair_apply add foreign key(r_state_no)
    references happy_home_repair.tb_repair_state(r_state_no) on delete cascade;
